...
Every event carries a stable `event_id` (generated by the integration, not the
HA event id), and the table is declared with
...
QuestDB deduplication makes the retry a no-op at the database level: the
delivery contract is at-least-once with an idempotent upsert, not end-to-end
exactly-once. The guarantee rests on one assumption - a legitimate state change
never repeats `(last_updated, entity_id)` - which is why that key, and not
`event_id`, defines identity. `event_id` remains available for analysis and
...
- proof that two legitimate state changes of one entity cannot share
  `(last_updated, entity_id)` - the assumption the dedup-based idempotency rests
